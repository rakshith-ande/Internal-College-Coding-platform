...
<body class="ui container" style="margin-top: 1%;">
	...
	<div><pre style="background:#f0f0f0;">
		...
		Contraints:
			1&nbsp;<&nbsp;=&nbsp;N&nbsp;<&nbsp;=&nbsp;10
			-10<sup>14</sup>&nbsp;<&nbsp;=&nbsp;a,b&nbsp;<&nbsp;=&nbsp;10<sup>14</sup>
		ouput:
			i) sum of a,b
	...
	<form method="post">
		...
		<textarea class="codemirror-textarea" name="code"  id="code"><?php echo $code ?></textarea><br><br>
		...
		<?php
			...
			if($run>=1){
				...
				$contest="Sum Prog";
				$que="select count(*) from scores_mock1 where userid='$uid' and contest='$contest' ";
				$s=$con->query($que);
				$count=$s->fetch_assoc();
				//print_r($count['count(*)']);
				if($count['count(*)']==0){
					file_put_contents("files/".$uid."_max_score.txt", $code);
					$sql = "insert into scores_mock1(userid,score,contest) values('".$uid."',".$score.",'".$contest."')";
					$r=$con->query($sql);
					$con->close();
				}else{
					$que="select score from scores_mock1 where userid='$uid' and contest='$contest' ";
					$s=$con->query($que);
					$sc=$s->fetch_assoc();
					if($sc['score']<$score){
						file_put_contents("files/".$uid."_max_score.txt", $code);
						...
						$sql="update scores_mock1 set score=$score,date='$date' where userid='$uid' and contest='$contest' ";
						$r=$con->query($sql);
					}
				}
				
				if($score==100){
					echo "<p style='color:green;'>Congractulations! All test cases passed</p>";
				...
				echo "<textarea disabled rows='2' cols='3'>$score</textarea><br><br>";
		?>
	</form>
